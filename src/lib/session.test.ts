...
describe("getSessionUser", () => {
  ...
  it("returns the user from /auth/me when a session cookie is present", async () => {
    cookiesMock.mockResolvedValue({ get: () => ({ value: "token" }) });
    apiFetchMock.mockResolvedValue({ user: { id: 1, name: "Aïcha Diallo", roles: ["mentee"] } });

    const { getSessionUser } = await import("./session");
    const user = await getSessionUser();

    expect(user).toMatchObject({ id: 1, name: "Aïcha Diallo" });
...
describe("primaryRole", () => {
  it("returns the first known role found on the user", async () => {
    const { primaryRole } = await import("./session");

    expect(primaryRole({ roles: ["mentee", "donor"] } as never)).toBe("mentee");
    expect(primaryRole({ roles: ["donor"] } as never)).toBe("donor");
  });

  it("returns null when the user has no recognized role", async () => {
    const { primaryRole } = await import("./session");

    expect(primaryRole({ roles: ["unknown"] } as never)).toBeNull();
  });
});
