  ...
  return (
    <>
      ...
      <section className="py-16">
        <Container>
          <div className="flex flex-wrap gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-border-default dark:bg-surface">
            {["Niveau", "Domaine", "Durée", "Langue", "Difficulté"].map((filter) => (
              <span
                key={filter}
                className="rounded-full bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-500 dark:bg-white/10 dark:text-slate-300"
              >
                {filter} ▾
              </span>
            ))}
        ...
        <Container className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {catalogue.map((module) => (
            ...
            >
              ...
              <h3 className="mt-3 text-lg font-bold text-stf-navy dark:text-white">{module.title}</h3>
        ...
        <Container>
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-3">
            {[
              ...
              >
                <h3 className="font-semibold text-stf-navy dark:text-white">{item.title}</h3>
